package com.tokentm.sdk.components.common.barcode;

import java.util.Arrays;

/**
 * @author lqx  E-mail:herolqx@126.com
 * @Description 表示一个2D的位矩阵。 在下面的函数自变量中，以及在整个通用函数中
 * 模块，x是列位置，y是行位置。 顺序始终是x，y。
 * 原点在左上角。
 * 内部，这些位以32位int的1-D数组表示。 但是，每一行开始
 * 具有新的int。 这是有意完成的，因此我们可以非常复制地将一行复制到BitArray中
 * 有效。
 * 位的顺序是行优先的。 在每个int中，最低有效位将首先使用，
 * 表示它们代表较低的x值。 这与BitArray的实现兼容.
 */
